...
fetch('data/links.json',{cache:'no-store'}).then(r=>r.json()).then(d=>{
 ...
 document.getElementById('links').innerHTML=(d.links||[]).filter(x=>x.enabled).map(x=>`<a class="link ${x.featured?'featured':''}" href="${esc(x.url||'#')}"><span class="icon">${esc(x.icon||'🔗')}</span><span class="copy"><b>${esc(x.title)}</b><small>${esc(x.subtitle)}</small></span><span class="arrow">›</span></a>`).join('');
