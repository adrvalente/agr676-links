...
async function loadUsers(){
  ...
  try{
    ...
    box.innerHTML=out.users.map(u=>`<div class="user-row"><div><b>${u.name||u.username}</b><small>${u.username}</small></div><span class="role-badge">${u.role}</span><div>${u.active?'Ativo':'Desativado'} ${u.username!==currentSession?.user?.username?`<button data-toggle-user="${u.username}" data-active="${u.active}">${u.active?'Desativar':'Ativar'}</button>`:''}</div></div>`).join('');
    box.querySelectorAll('[data-toggle-user]').forEach(b=>b.onclick=async()=>{await api('/api/users/toggle',{method:'POST',body:JSON.stringify({username:b.dataset.toggleUser,active:b.dataset.active!=='true'})});loadUsers()});
  }catch(e){box.textContent='Não foi possível carregar utilizadores: '+e.message}
}
